	...
	return (
		<>
			...
			<div className='flex gap-1 w-2/3 ml-auto'>
				<div className={`block  z-20 max-w-fit px-2 pt-1 rounded-md shadow-md ml-auto relative ${bgClass}`}>
...
const VideoMessage = ({ message }: { message: IMessage }) => {
	return <ReactPlayer url={message.content} width='250px' height='250px' controls={true} light={false} />;
};
...
const MessageTime = ({ time, fromMe }: { time: string; fromMe: boolean }) => {
	return (
		<p className='text-[10px] justify-end mt-2 self-end flex gap-1 items-center'>
